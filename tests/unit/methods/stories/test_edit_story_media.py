...
async def test_supports_streaming_reaches_the_media_whichever_branch_built_it(
    ...
    *,
    ...
    # Same omission as the disk branch of `send_story`: a replacement video uploaded from
    #  a path was posted non-streamable however the parameter was set.
    with pytest.raises(InvokeCalled) as exc_info:
        await story_client.edit_story_media(
            chat_id=7,
            story_id=1,
